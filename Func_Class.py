...
class Excel():
    ...
    def __init__(self,name, PS, email):
        self.name = name
        self.PS = PS
        self.email = email
        #self.data = data
        ...
        # ws=wb.get_sheet_by_name('Sheet1')
            ...
            for i in range(1, s + 1):
                if ws.cell(row=i, column=1).value == self.name and ws.cell(row=i, column=2).value == self.PS \
                        and ws.cell(row=i, column=3).value == self.email:
                    if sheet == "Sheet0":
                        break
                    if sheet != 'Sheet1':
    ...
    def data_write(self, data,found):
        self.data = data
        self.found = found
        path = "D:\Python_Practice\LnT.xlsx"
        wb = load_workbook(path)
        if self.found == 1:
            if 'Sheet0' not in wb.sheetnames:
                ...
                for i in range(1, len(head)+1):
                    ws.cell(row=s + 1, column=i).value = self.data[i - 1]
                wb.save(path)
            else:
...
#write = Excel(name, PS, email, data, found)
write = Excel(name, PS, email)
